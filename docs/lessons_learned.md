...
- 🔴 **Freeform marketplace text is not an authority signal.** CAPTCHA and purchased words can appear in titles, descriptions, or injected row text. CAPTCHA requires both semantic and structural challenge evidence. Purchased text may remove a tracked product only when it belongs to a structurally complete, identity-validated wishlist reconciliation; standalone product scrapes and partial wishlist reads remain non-destructive, and price history is retained.
...
- 🔴 **Amazon wishlist pagination is not always an `/ls/{id}` URL.** Amazon.nl uses same-origin `wishlist/slv/items` fragments with `lid` and an opaque token, then emits the same endpoint with an empty token as its terminal sentinel. Bind each continuation to the original list and origin, accept non-empty tokens as pages, treat the empty-token sentinel as terminal, and use the validated fragment URL as identity evidence when the fragment omits document metadata.
- 🔴 **A full-count mock can hide a broken pagination boundary.** The v1.4.3 source-selection contract injected a successful 784-item background result, so it proved which source won but never exercised Amazon's next-link URL policy or the offscreen fragment parser. Keep source-selection tests, but pair Amazon pagination changes with a cross-layer scraper/parser fixture and a sanitized public-list release canary; do not use the live canary as flaky CI.
