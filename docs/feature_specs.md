...
- **Features:**
  ...
  - Chart summary chips show the current price, change since the first retained sample, low/high with the date each run began, target distance or reached state, and stored-sample and price-change counts. Below the chart, consecutive samples at the same price are collapsed into up to six newest-first "price changes" rows; older runs remain available through "View all entries".
