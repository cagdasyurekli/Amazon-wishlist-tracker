...
- **Full Manual Reads:** Manual import and sync use the bounded background paginator as the authoritative source so Amazon's initially rendered 10-row DOM cannot truncate a large wishlist. Identity-bound visible rows are used only as an explicitly partial fallback when the background read fails or is paused.
- **Amazon Continuation URLs:** Background pagination accepts Amazon's exact `wishlist/slv/items` endpoint only when it stays on the same origin and carries one bounded matching `lid` plus one non-empty pagination token. The same endpoint with an empty token and matching identity is a terminal sentinel, not another network request. Identity-bound continuation fragments can establish their list identity from the already validated final response URL, but unstructured or mismatched responses remain indeterminate.
...
- **Features:**
  ...
  - History charts plot price as a step line on a true time axis: a price holds until the next sample with a different price, so plateau width reflects elapsed time rather than sample count. Axes use rounded price ticks and date labels scaled to the visible span. Low, high, and current points are labelled, and a finite target price is drawn as a dashed reference line when it lies within 50%–150% of the observed range.
  - Hovering, touching, or focusing the chart and using Left/Right arrow keys reveals a tooltip for each held-price run with its price, start and end, duration, check count, and change versus the previous price. Escape dismisses it. The chart exposes a text summary to assistive technology.
  - Chart summary chips show the current price, change since the first retained sample, low/high with the date each run began, target distance or reached state, and stored-sample and price-change counts. Below the chart, consecutive samples at the same price are collapsed into up to six newest-first "price changes" rows; older runs remain available through "Full price history".
  - Each product card shows the same since-tracking change badge as the popup (baseline, 0.5% threshold, and tooltip wording) beside the current price, and marks a met target price as reached.
  - "Full price history" (labelled "Product details" when no history exists) opens the details view, which lists retained history as newest-first held-price periods with start/end, duration, check count, and change versus the previous period. At most 500 periods are listed; older ones remain in storage and backups.
  ...
  - Price display formats the numeric portion for the browser locale while preserving the stored currency symbol. Recognized currencies use their own minor units, so yen prices show no decimals. History charts label their point count as stored samples, not fetches.
