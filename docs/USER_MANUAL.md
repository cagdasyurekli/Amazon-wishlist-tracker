...
2. Select the chart icon or **Open Dashboard**. The dashboard should open in a tab.
...
3. Select it and watch the button state: **Adding…** while the request is in
   ...
   **⚠️ Try Again** if it fails. The in-page control does not show a separate
...
3. You can also paste a supported wishlist URL into **Paste Amazon Wishlist URL** and
   select **Import**.
4. In **Select Items**, keep or clear the products you want. Use **Select All** when
   helpful.
5. Decide whether to enable **Keep List in Sync**.
...
Open the dashboard with the chart icon or **Open Dashboard** in the popup.
...
- Select **Details** for current price, stock, target, wishlist discount information,
  and timestamped history.
- Select **Chart** for one product or **Show All Charts** for all rendered products.
...
Select the empty star (**☆**) to add a product to Priority Tracking; a filled star
(**⭐️**) means it is active. Select the filled star to remove priority. Up to 10
products can be priority items at once.
...
- A wishlist item detected as purchased; that item is removed from tracking.
...
Selecting a price, discount, restock, or purchased-item notification opens the tracked
Amazon product. Selecting the previous-target upgrade notice opens **Extension
Settings** instead.
...
Select **Export Data (JSON)** to download `amazon_tracker_export.json`. It contains the
...
inside the extension. Export first if you need a record. If a background check is
already finishing, the clear waits behind it so older samples are not restored after
the confirmation. A failed request leaves the button available for another attempt.
...
- **notifications**: show price, discount, restock, and purchased-item alerts.
...
| Import a wishlist | Popup or dashboard | **Import This Wishlist**, **Track This Wishlist**, or paste URL + **Import** |
| Refresh a tracked wishlist | Dashboard with the list open | **Sync Wishlist Now** |
| Open all products | Popup | Chart icon or **Open Dashboard** |
| Set a target | Dashboard product card | **Edit** → price → **Save** |
| Prioritize a product | Dashboard product card | **☆** to enable, **⭐️** to disable |
| Inspect history | Dashboard product card | **Details** or **Chart** |
